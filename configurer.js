...
function Configurer(grunt, _dirname) {
    ...
    function handleConfig(_config, config, defaultConfig) {
        Object.keys(_config).forEach(function (key) {
            if (config[key]) {
                ...
                config = handleConfigAdditions(key, _config, config, defaultConfig);
            } else {
    ...
    /**
     * If handle config "merging" we don't merge we only add options.
     * If the defaultConfig is passed we don't stop the grunt process we just ignore it.
     * This way you can set extra parameters to the path object in a configurer.
     */
    function handleConfigAdditions(key, _config, config, defaultConfig) {
